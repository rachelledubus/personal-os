...
update roadmap_items set title = 'Build CRM Foundation'
  where user_id = auth.uid() and title = 'Build CRM: contact categories, lead sources, pipeline stages';

insert into milestones (user_id, roadmap_item_id, title, sort_order)
select auth.uid(), id, sub.title, sub.ord
...
update roadmap_items set title = 'Complete CRM + Publish Content #1'
  where user_id = auth.uid() and title = 'Build CRM: follow-up rules + tracking complete. Publish flagship content #1.';
...
update roadmap_items set title = 'Draft Real Payment Guide + Content #2'
  where user_id = auth.uid() and title = 'Real Payment Guide: outline + draft content. Publish flagship content #2.';
...
update roadmap_items set title = 'Launch Real Payment Guide + Month 1 Review'
  where user_id = auth.uid() and title = 'Real Payment Guide live + promotion. Relocation funnel outlined. Email nurture drafted. Publish flagship content #3. Month 1 review Friday.';
...
update roadmap_items set title = 'Cooper City Guide + Content #4'
  where user_id = auth.uid() and title = 'Cooper City guide drafted. Email nurture finalized. Publish flagship content #4.';
...
update roadmap_items set title = 'Plantation Guide + Network List'
  where user_id = auth.uid() and title = 'Plantation guide drafted. List 10 target organizations for 05D. Publish flagship content #5.';
...
update roadmap_items set title = 'Pembroke Pines Guide + Outreach Begins'
  where user_id = auth.uid() and title = 'Pembroke Pines guide drafted. Begin outreach — first 5 organizations. Publish flagship content #6.';
...
update roadmap_items set title = 'Consultation Materials + Month 2 Review'
  where user_id = auth.uid() and title = 'Consultation presentation + buyer questionnaire built. Outreach to remaining 5 organizations. Publish flagship content #7. Month 2 review Friday.';
...
update roadmap_items set title = 'Conversion Data Review'
  where user_id = auth.uid() and title = 'Pull data: which content created conversations, which relationships created opportunities. Publish content #8.';
...
update roadmap_items set title = 'Activity vs. Results Diagnosis'
  where user_id = auth.uid() and title = 'Diagnose per System 10 rules: activity vs. results. Publish content #9.';
...
update roadmap_items set title = 'Scale Top Lead Source'
  where user_id = auth.uid() and title = 'Expand: double down on the top lead source. Publish content #10.';
...
update roadmap_items set title = 'Deepen Top Partner Relationships'
  where user_id = auth.uid() and title = 'Expand: deepen the 2-3 most responsive organizations. Publish content #11.';
...
update roadmap_items set title = 'Day-90 Review + Next Plan'
  where user_id = auth.uid() and title = 'Repurpose top-performing content. Day-90 review against Phase 1 success criteria. Set next 90-day plan Friday.';
