...
/** Waves off any block from today's schedule — not a completion, just
 *  "stop showing me this today." Only affects today's generated row,
 *  never the underlying weekly template, so tomorrow's schedule is
 *  unaffected. */
export async function dismissBlock(blockId) {
  const { error } = await supabase.from('time_blocks').update({ dismissed: true }).eq('id', blockId);
  if (error) throw error;
}
