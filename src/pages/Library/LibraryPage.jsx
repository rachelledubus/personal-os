...
const TABS = ['reference', 'documents', 'finances', 'notes'];
      ...
      {filtered.length === 0 ? <EmptyState icon="leaf" title="No matches" /> : (
        <div className="stack">
          {filtered.slice(0, 30).map(i => (
            <details key={i.id}>
              <summary style={{ fontWeight: 700, cursor: 'pointer', padding: '6px 0' }}>{i.title}</summary>
  ...
  return (
    <Card>
      ...
      {bills.length === 0 ? <EmptyState icon="leaf" title="No bills added yet" /> : (
        <div className="stack">
          {bills.map(b => (
            <div key={b.id} className="row-between" style={{ padding: '8px 0' }}>
  ...
  return (
    <Card>
      ...
      <div className="stack" style={{ marginTop: 'var(--space-5)' }}>
        {notes.map(n => <div key={n.id} className="muted" style={{ fontSize: 13, borderBottom: '1px solid var(--sand)', padding: '8px 0' }}>{n.content}</div>)}
      </div>
    </Card>
  );
}
